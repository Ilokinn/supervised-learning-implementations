...
def get_dbscan_model(MIN_SAMPLES=5,EPS=0.5):
    # EPS la distance maximale ou le rayon du voisinage 
...
# EXEMPLE D'UTILISATION - K-MEANS X = vos données (n_samples, n_features)
# 1. Créer le modèle

# kmeans = get_kmeans_model(N_clusters=3, random_state=42)

# 2. Fit sur les données

# labels_kmeans = kmeans.fit_predict(X) 
# OU
# kmeans.fit(X)
# labels_kmeans = kmeans.labels_  # Récupérer les labels après fit

# 3. Accéder aux attributs

# print("Centres des clusters:", kmeans.cluster_centers_)
# print("Inertia:", kmeans.inertia_)
# print("Labels:", labels_kmeans)
...
# EXEMPLE D'UTILISATION - DBSCAN

# 1. Créer le modèle
# dbscan = get_dbscan_model(MIN_SAMPLES=5, EPS=0.5)
...
# print("Nombre de clusters:", len(set(labels_dbscan)) - (1 if -1 in labels_dbscan else 0))
# print("Nombre de points bruit (label=-1):", np.sum(labels_dbscan == -1))

# 4. Évaluer le modèle (seulement si pas trop de bruit)
...
# else:
#     print("Trop de bruit pour calculer silhouette")
# ============================================================================
# EXEMPLE D'UTILISATION - SPECTRAL CLUSTERING
# ============================================================================

# 1. Créer le modèle
# spectral = get_spectral_model(N_clusters=3, Random_State=42)

# 2. Fit et predict
# labels_spectral = spectral.fit_predict(X)  
# 3. Accéder aux attributs
# print("Labels:", labels_spectral)
# print("Nombre de clusters:", len(set(labels_spectral)))

# 4. Évaluer le modèle
# silhouette_spectral = silhouette_score(X, labels_spectral)
# davies_bouldin_spectral = davies_bouldin_score(X, labels_spectral)
# print(f"Silhouette Score: {silhouette_spectral:.3f}")
# print(f"Davies-Bouldin Score: {davies_bouldin_spectral:.3f}")